...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        switch (status) {
            case "pending":
                ...
                holder.btnHoanHang.setVisibility(View.GONE);
                holder.btnHuyHang.setVisibility(View.VISIBLE);
                break;
            case "cancelled":
            case "returned":
            case "shipped":
                holder.btnChiTiet.setVisibility(View.GONE);
                holder.btnHoanHang.setVisibility(View.GONE);
                holder.btnHuyHang.setVisibility(View.GONE);
                break;
            default:
                holder.btnChiTiet.setVisibility(View.VISIBLE);
                holder.btnHoanHang.setVisibility(View.VISIBLE);
                holder.btnHuyHang.setVisibility(View.GONE);
                break;
        }
        ...
        holder.btnHuyHang.setOnClickListener(v -> {
            new androidx.appcompat.app.AlertDialog.Builder(context)
                    .setTitle("Xác nhận")
                    .setMessage("Bạn có chắc muốn hủy đơn hàng này không?")
                    .setPositiveButton("Hủy đơn", (dialog, which) -> {
                        requestCancelOrder(order.getOrder_id(), holder.getAdapterPosition());
                    })
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        Button  btnChiTiet, btnHoanHang, btnHuyHang;
        public ViewHolder(View itemView) {
            ...
            btnHuyHang = itemView.findViewById(R.id.btnHuyHang);
        }
    }
    ...
    private void requestCancelOrder(int orderId, int position) {
        ApiOrder apiOrder = RetrofitProduct.getClient().create(ApiOrder.class);

        apiOrder.cancelOrder(orderId).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                if (response.isSuccessful()) {
                    Toast.makeText(context, "Hủy đơn hàng thành công!", Toast.LENGTH_SHORT).show();
                    orderList.get(position).setStatus("cancelled"); // cập nhật trạng thái
                    notifyItemChanged(position);
                } else {
